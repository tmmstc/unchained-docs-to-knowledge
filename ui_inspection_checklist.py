...
def analyze_ui_structure(file_path="streamlit_app.py"):
    ...
    if title_match:
        ui_elements.append(("st.title", main_content.find(title_match.group(0), main_start), title_match.group(1)))
    
    # st.header - Select Directory
    header_match = re.search(r'st\.header\("Select Directory"\)', main_content)
    if header_match:
        ui_elements.append(("st.header", main_content.find(header_match.group(0), main_start), "Select Directory"))
    ...
    dir_check = re.search(r'if directory_path and os\.path\.exists\(directory_path\):', main_content)
    if dir_check:
    ...
    after_checkbox = content[checkbox_start:checkbox_start+500]
    
    # Check for conditional statements wrapping checkbox
    if_count = before_checkbox.count('if ')
    close_paren = after_checkbox.count(')')
    ...
    summary_logic = re.search(
        r'if summary:.*?else:.*?if SUMMARIZER_AVAILABLE',
        display_func_content,
        re.DOTALL
    )
    ...
    if 'if directory_path' in checkbox_context:
        issues.append("Checkbox may be inside directory_path conditional (won't show until path entered)")
