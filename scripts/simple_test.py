...
def start_backend_process():
    ...
    try:
        ...
        process = subprocess.Popen(
            [str(venv_python), "start_backend.py"],
...
def test_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"ERROR: Root endpoint returned " f"{response.status_code}")
        ...
        if response.status_code == 200:
            ...
            print(f"ERROR: Stats endpoint returned " f"{response.status_code}")

        print("\nAPI tests completed successfully!")
        print("Backend is working and can be accessed at " "http://127.0.0.1:8000")
        print("You can now manually start Streamlit at " "http://localhost:8501")
...
if __name__ == "__main__":
    ...
    backend_thread = threading.Thread(target=start_backend_process, daemon=True)
